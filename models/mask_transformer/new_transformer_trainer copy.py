...
class MaskTransformerTrainer:
    def __init__(self, args, t2m_transformer, vq_model, cond_mode=None):
        self.opt = args
    ...
    def forward(self, batch_data):
        ...
        word_embeddings, pos_one_hots, pos_indices, sen_embedding, caption, sent_len, motion, m_lens, tokens = batch_data
        
        conds = caption
        word_embeddings = word_embeddings.detach().float().to(self.device)
        pos_indices = pos_indices.detach().long().to(self.device)
        sen_embedding = sen_embedding.detach().float().to(self.device)
    ...
    def train(self, train_loader, val_loader, eval_val_loader, eval_wrapper, plot_eval):
        ...
        best_fid, best_div, best_top1, best_top2, best_top3, best_matching, writer = new_evaluation_mask_transformer(
            self.opt.save_root, eval_val_loader, self.t2m_transformer, self.vq_model, self.logger, start_epoch,
        ...
        wandb_init("MOMENT-M-Transformer", config=self.opt, id=self.opt.name)

        # while epoch < self.opt.max_epoch:
        for epoch in tqdm(range(start_epoch, self.opt.max_epoch), desc="Train"):
            ...
            for i, batch in enumerate(train_loader):
                ...
                logs['loss'] += loss; total_loss.append(loss)
                logs['acc'] += acc; total_acc.append(acc)
            ...
            wandb.log({
                ...
                "Train Acc":  np.mean(total_acc),
                "Validation Loss": np.mean(val_loss),
                "Valiation Acc": np.mean(val_acc),
            })
...
class ResidualTransformerTrainer:
    ...
    def train(self, train_loader, val_loader, eval_val_loader, eval_wrapper, plot_eval):
        ...
        wandb_init("MOMENT-R-Transformer", config=self.opt, id=self.opt.name)
