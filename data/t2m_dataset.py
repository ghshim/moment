...
class NewText2MotionDataset(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer):
        self.opt = opt
        self.w_vectorizer = w_vectorizer
        self.max_length = 20
        ...
        for name in tqdm(id_list):
            try:
                motion = np.load(pjoin(opt.motion_dir, name + '.npy'))
                clip_embedding = np.load(pjoin(self.clip_emb_dir, name + '.npy'))
                ...
                with cs.open(pjoin(opt.text_dir, name + '.txt')) as f:
                    for i, line in enumerate(f.readlines()):
                        ...
                        text_dict['caption'] = caption
                        text_dict['tokens'] = tokens
                        text_dict['embedding'] = clip_embedding[i]
    ...
    def __getitem__(self, item):
        ...
        # Randomly select a caption
        text_data = random.choice(text_list)
        caption, tokens, embedding = text_data['caption'], text_data['tokens'], text_data['embedding']
...
class Text2MotionLenDataset(data.Dataset):
    def __init__(self, opt, mean, std, split_file):
        self.opt = opt
        self.max_length = 20
        self.pointer = 0
    ...
    def __getitem__(self, item):
        ...
        # print(word_embeddings.shape, motion.shape)
        # print(tokens)
        return caption, motion, m_length, embedding
